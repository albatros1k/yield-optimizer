...
import { valkAddressBook as addressBook } from '../src/config/valkAddressBook'
...
const oldValidOwners = [
  addressBook.arbitrum.platforms.valk.devMultisig,
];
...
const validateSingleChain = async (chainId, uniquePoolId) => {
  ...
  poolsWithStrategyData.forEach(pool => {
    ...
    const { keeper, strategyOwner, vaultOwner, beefyFeeRecipient, beefyFeeConfig } =
      addressBook[chainId].platforms.valk;
...
const populateVaultsData = async (
  ...
  const multicall = new MultiCall(web3, addressBook[chain].platforms.valk.multicall);
