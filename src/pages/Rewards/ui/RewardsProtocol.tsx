...
import { IRewardPosition } from '../types/index';
...
export const RewardsProtocol: FC<{ rewardsPosition: IRewardPosition }> = ({
  ...
  const {
    colors: { alterText, alterHelp, alterBg, subAccentMain },
  } = useTheme();

  const renderTokens = (): JSX.Element[] =>
    reward.map(({ symbol, value, valueUSD, address }, index: number, { length }) => (
  ...
  return (
    <Card w="100%" p="30px 25px" m="0 0 24px">
      <Row m="0 0 20px" w="100%" align="center">
        ...
        />
        <H3 m="0 15px 0 0">{parseProtocol(protocolName)} Rewards</H3>
        <H3 color={alterText}>{definePlus(totalRewards, false)}</H3>
        {isSupported && isAbleToClaim ? (
          <Button w="140px" h="28px" m="0 20px 0 auto" bg="transparent" borderColor={subAccentMain}>
            Claim All
          </Button>
        ) : null}
      </Row>
